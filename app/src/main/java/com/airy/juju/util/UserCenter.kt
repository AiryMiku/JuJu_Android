...
import com.airy.juju.Common.SharedPreferenceKey.USER_TOKEN
...
fun isLogin(context: Context): Boolean {
    return getSp(context).getString(Common.SharedPreferenceKey.USER_TOKEN, "")?.isNotBlank() ?: false
}

fun getUserId(): String = getSp(getApplicationContext()).getString(USER_ID, "") ?: ""

fun getUserToken(): String = getSp(getApplicationContext()).getString(USER_TOKEN, "") ?: ""

fun logout() {
    getSp(getApplicationContext()).edit().putString(Common.SharedPreferenceKey.USER_ID, "").apply()
    getSp(getApplicationContext()).edit().putString(Common.SharedPreferenceKey.USER_TOKEN, "").apply()
}

fun saveLogin(context: Context, userId: String, userToken: String) {
    getSp(context).edit().putString(Common.SharedPreferenceKey.USER_TOKEN, userToken).apply()
